...
class SyncAndDetect {
 ...
 private:
  ...
  class Subscriber {
   public:
    ...
    Subscriber(const std::vector<std::string> &imageTopics,
      ...
      const std::vector<std::vector<std::string>> tpv = {imageTopics,
                                                         odomTopics};
      auto cb = std::bind(&SyncAndDetect::processImages, sand,
      ...
      for (size_t i = 0; i < imageTopics.size(); i++) {
        views_.emplace_back(new View<SyncT>(imageTransport_.get(),
                                            ...
                                            subQueueSize, imagesAreCompressed));
      }
   ...
   private:
    ...
    class View {
     public:
      View(ImageTransport *it, const std::string &topic, VSyncT *sync, int qs,
           bool useCompressed)
          : topic_(topic), sync_(sync) {
        image_transport::TransportHints th(useCompressed ? "compressed"
                                                         : "raw");
    ...
    class Odom {
     public:
      Odom(ros::NodeHandle &nh, const std::string &topic, OSyncT *sync, int qs)
          : topic_(topic), sync_(sync) {
  ...
  std::vector<ros::Publisher> imagePubs_;
