...
bool SyncAndDetect::initialize() {
  ...
  if (tagFamily != Family::tf36h11 && tagFamily != Family::tf25h9 &&
  ...
  if (runOnline()) {
    ...
    if (annotateImages_) {
      for (const auto &outputTopic : imageOutputTopics_) {
        imagePubs_.push_back(nh_.advertise<sensor_msgs::CompressedImage>(
            outputTopic, pubQueueSize));
      }
    }
  ...
  if (imagesAreCompressed_) {
    if (useApproximateSync_) {
      iterate_through_bag<ApproximateCompressedSync, CompressedImage>(
          ...
          std::bind(&SyncAndDetect::processCompressedImages, this,
    ...
    if (useApproximateSync_) {
      iterate_through_bag<ApproximateSync, sensor_msgs::Image>(
          imageTopics_, odometryTopics_, &view, &outbag_, syncQueueSize_,
          std::bind(&SyncAndDetect::processImages, this, std::placeholders::_1,
                    std::placeholders::_2));
    } else {
