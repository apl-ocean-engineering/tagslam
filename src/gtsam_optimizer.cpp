...
{
  return (rclcpp::get_logger("gtsam_optimizer"));
}
...
FactorKey GTSAMOptimizer::addRelativePosePrior(
...
FactorKey GTSAMOptimizer::addDistanceMeasurement(
  const double d, const double noise, Eigen::Vector3d corner1, ValueKey T_w_b1,
  ValueKey T_b1_o, Eigen::Vector3d corner2, ValueKey T_w_b2, ValueKey T_b2_o)
{
...
std::vector<FactorKey> GTSAMOptimizer::addTagProjectionFactor(
  ...
  const Eigen::Matrix<double, 4, 3> & objCorners, const string & camName,
...
double GTSAMOptimizer::checkForLargeErrors(double thresh) const
{
  ...
  for (const auto & i : fullGraph_) {
    ...
    if (e > thresh) {
      LOG_DEBUG("graph has large err: " << i->error(values_));
...
{
  LOG_DEBUG(
    "incremental optimize new values: " << newValues_.size()
  ...
  if (newGraph_.size() > 0) {
    ...
    for (int i = 0; i < maxIter_ - 1; i++) {
      ...
      if (
        *res.errorAfter < lastError_ + deltaError ||
        fabs(*res.errorAfter - prevErr) < 0.01) {
        LOG_DEBUG(
          "stopped after iter  "
          << i << ", changed: " << lastError_ << " -> " << *res.errorAfter
          << " = " << *res.errorAfter - lastError_
          << " last iter: " << *res.errorAfter - prevErr);
        break;
      } else {
        LOG_DEBUG(
          "it " << i << " new err: " << *res.errorAfter << " vs last: "
                << lastError_ << " +delta: " << lastError_ + deltaError);
      }
...
#ifdef DEBUG
static void print_large_errors(
  const string & label, const gtsam::ExpressionFactorGraph & g,
...
{
  LOG_DEBUG(
    "optimizing full(" << force << ") new fac: " << newGraph_.size()
                       << ", new val: " << newValues_.size());
  if (newGraph_.empty() && newValues_.empty() && !force) {
    LOG_INFO("graph not updated, no need to optimize!");
    return (lastError_);
  }
  fullGraph_ += newGraph_;
...
KeyToErrorMap GTSAMOptimizer::getErrors(
  const std::vector<FactorKey> & keys) const
{
  KeyToErrorMap ke;
  ...
  for (const auto & k : keys) {
    try {
      ...
      LOG_WARN("cannot get error for key: " << k);
    }
  }
  return (ke);
}

void GTSAMOptimizer::printFactorError(FactorKey k) const
{
  gtsam::ExpressionFactorGraph testGraph = fullGraph_;
  testGraph += newGraph_;
  gtsam::Values testValues = values_;
  ...
  std::cout << " factor: " << std::endl;
  i->print();
  std::cout << std::endl << "   corresponding values: " << std::endl;
